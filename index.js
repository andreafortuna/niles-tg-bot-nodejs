...
bot.start((ctx) => {
    ctx.reply(`Hello ${ctx.message.from.first_name} ${ctx.message.from.second_name}`)
})
bot.help((ctx) => {
...
bot.command('test', (ctx) => {
    ctx.reply(`Hello ${ctx.message.from.first_name} ${ctx.message.from.second_name}, this is a TEST!`)
})
